...
def crawl_cargonews():
    ...
    with sync_playwright() as p:
        ...
        print("CargoNews 메인 페이지 접속 중...")
        # 1. 메인 목록 페이지 접속
        page.goto("https://www.cargonews.co.kr/news/articleList.html?view_type=sm")
        page.wait_for_load_state("domcontentloaded")
        
        # 2. 전체 기사 리스트 section-list 내에서 링크 5개 추출
        # 사용자 요청에 따라 section-list 하위의 a 태그들을 가져옵니다.
        ...
        for a_tag in items:
            ...
            if href:
                ...
                # 기사 상세 페이지 링크인지 확인 (보통 /news/articleView.html?idxno= 등)
                ...
                if header_el.is_visible():
                    ...
                    for li in date_els:
                        ...
                        if "기자" not in li_text and ("-" in li_text or "." in li_text or ":" in li_text):
                            # 보통 날짜에 구분자가 들어감 (예: 2026.03.01 또는 2026-03-01)
                            date = li_text
                            break
...
if __name__ == "__main__":
    print("CargoNews 크롤러 시작...")
    crawled_data = crawl_cargonews()
    
    output_file = 'cargonews_result.json'
